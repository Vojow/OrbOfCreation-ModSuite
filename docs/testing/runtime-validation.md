...
1. **Quick controls (Q):** with Auto Cast off and emergency clear, keep the AC control and Mods Runtime card visible. Click once and confirm exactly one saved transition, with the setting, button, and configured-status axis all becoming On and no intervening or release-time Off repaint. Repeat with `F8`, then repeat On-to-Off. Engage emergency stop and confirm desired state remains `ON / STOPPED` while healthy, lifecycle-not-ready, and host-fault status is simulated or observed. A stale button with a current Runtime card isolates Unity control lifetime/rendering; both stale while cycle evidence advances requires a host trace.
