...
- On 2026-07-19, all 744 supported game-independent behavior and knowledge-map tests passed with `UseGameStubs=true` on the combined invalidation, structured-decision, feature-health, circuit-breaker, queue-acceptance, action-family-ownership, and versioned-configuration stack.
- The checked-in coverage gate passed at 73.07% overall production line coverage: Automata 72.79%, Mentor 72.11%, Mod Config 29.19%, and Orb Modding Common 89.51%.
...
Version `0.8.9` replaces the overlapping action-multiplier, affordable-repeat, and Structure-repeat controls with `PurchaseGrouping`. Grouping and continuation are independent: every ranked Structure receives its configured `Single`, `Fixed`, `BulkDevelopment`, or `ActionMultiplier` group, Upgrades remain one level except in `ActionMultiplier`, and the ranked pass repeats while queue quota and live admission permit. Definite pre-mutation rejection advances to healthy lower ranks and retries on bounded delay; attempted or ambiguous mutation remains lifecycle-quarantined.

Version `0.8.10` permits up to 16 exact one-level Auto Buy calls inside one mutation-owning coordinator lease, while retaining the 1 ms purchase slice. Before promotion, capture the burst-size distribution and native call duration on desktop and Steam Deck/Proton under a rapidly consumed queue. Confirm each level re-reads live cost, reserve, availability, lifecycle, ownership, and authoritative queue room; queue depth never exceeds capacity or consumes the configured manual reserve; Bulk Development advances to the next ranked candidate after its live group; and finite Upgrades remain one level per pass outside `ActionMultiplier`. During the run, change capacity, trigger emergency disable, and cross a reserve threshold while a burst is eligible, then confirm no additional stale purchase occurs after the changed condition is observed. Auto Cast, Auto Concept, Spell Leveling, and Mentor must not mutate during an Auto Buy lease, and their coordinator wait counters must recover after the lease. Inspect UI popup/tween behavior, synchronous queue/completion callback ordering, Upgrade native multi-buy restoration, frame-time outliers, ambiguous-failure quarantine, and save/load generation cancellation. The installed runtime gate remains pending until both platform captures are reviewed.
...
For versioned configuration schemas, close the game before every file change and work only with preserved disposable configuration copies. Exercise each supported plugin through these exact startup states: fresh/missing file, existing schema-zero file, current version 1, malformed or negative marker/value failure, and future version 2. A fresh file has no original bytes to back up; an existing schema-zero file must create the first free sibling `*.pre-schema-v1.bak` before mutation, and a pre-existing backup must remain byte-identical while the `.bak.2` sibling is selected. Confirm the backup exactly matches the original length, bytes, and SHA-256; an induced write/flush failure must leave no owned partial backup and start no plugin behavior. Current version 1 must create no backup and perform no schema save. Automata must apply only the documented mode/interval/obsolete-key mapping; Mentor and Mod Config must retain their typed values. Failed and future files must remain byte-identical, load no affected plugin behavior, and publish `Failed` or `Future` rather than runtime-health success. At 1280×720, 1280×800, and 1920×1080, select each exact plugin in Mods and confirm Current, Migrated, Failed, and Future schema text is readable in its own band, saved/loaded and backup claims match the file outcome, a same-session late status transition refreshes, and neither the band nor logs contain a configuration path or serialized value. Failed/future Automata or Mentor instances with no bound visible settings must still appear by exact GUID as read-only status-only tabs with no setting rows or Apply action; an unreported empty third-party plugin must remain omitted. Restore every original config byte-for-byte after the matrix.

For unified feature health, confirm each gameplay control and tooltip distinguishes saved `OFF`, progression `Locked`, lifecycle `Not ready`, normal `Operational`, recoverable queue/safety blocking, unavailable contract, degradation, and fault evidence without changing the underlying queue or XP work. At 1280x720, 1280x800, and 1920x1080, inspect Automata Auto Buy, Auto Cast, Auto Concept, and Mentor with both short and wrapped reasons; every status and decision line must have normal row spacing, with no text crossing a separator, another field, or the tooltip boundary. Enter and leave Main, load another save, and trigger the available reset/NG+ boundary; status generation must recover without retaining the previous save's reason. In Mentor, deliberately disable or make one optional domain unavailable while another is operational and confirm only the affected domain fails and the root reports degradation. In Orb Mod Config, verify the selected plugin's runtime band is visually separate from Apply/Revert feedback at each supported resolution, changes after the same runtime transition, and shows third-party plugins without publishers as `Not reported` rather than inventing health. Saving a setting must say only that configuration was saved.
