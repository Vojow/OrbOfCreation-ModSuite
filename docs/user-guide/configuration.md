...
- Auto Concept considers every discovered (unlocked) concept. While a settled training period is still running, its tooltip and Runtime status say that it is waiting. If training has finished but no other unlocked concept can be assigned, they show that progression-locked reason instead of treating a locked concept as a candidate. The decision journal and trace dashboard record the same idle reason.
