...
The suite is one plugin with one identity (`dev.vojow.orbofcreation.modsuite`), so BepInEx keeps all of it in one configuration file. That file carries a hidden `[OrbModding] ConfigurationSchemaVersion` marker and its current version is 7. Version 3 moved inherited shortcut defaults; version 4 removed Mentor's obsolete per-frame and CPU-budget controls; version 5 removes the retired scan cap, rejection cap, both global logging keys, Mentor detailed logging, mastery event probe, and verifier shortcut; version 6 shortens the inherited Auto Concept idle fallback from 300 to 10 seconds; version 7 shortens its inherited training period from 300 to 30 seconds. Player-customized intervals and shortcuts other than that retired verifier shortcut are preserved. Settings from the retired per-plugin files do not carry over. Before migration the transaction creates the first free sibling backup (`.pre-schema-v<version>.bak`, then `.bak.2`, and so on) and never overwrites one that exists. Malformed, negative, or newer unsupported schema versions stop the suite before it changes gameplay or opens its UI; do not lower the marker by hand. The Mods panel reports the schema state separately from both runtime health and the Apply result, and never displays the configuration path or saved values in that status.
...
- `AutoConcept.TrainingPeriodSeconds`: settled active time for one newly assigned concept; default 30, range 10 to 3600. Schema 7 moves the inherited former default of 300 seconds to 30 while preserving other customized values. `RotateAll` and `PreserveManual` can resume earlier after mastery catch-up, while `TimedCycle` always waits for the full period.
- Auto Concept considers only discovered (unlocked) concepts that pass its allow/block filters. While a settled training period is still running, its tooltip and Runtime status say that it is waiting. If training has finished but no other unlocked, allowed concept can be assigned, they show that progression-locked reason instead of treating a locked concept as a candidate. If the game refuses an unlocked replacement because its prospective slot or resource drain is not currently safe, Auto Concept tries the next unlocked candidate, defers the refused one for the fallback interval, and highlights that temporary retry state. Safe quantity can still be added to the active concept while replacements are deferred. The decision journal and trace dashboard record the same idle reason, while `BepInEx/LogOutput.log` names each verified quantity delta or rejected rotation and its native reason.
...
- `AutoConcept.FallbackEvaluationIntervalSeconds`: Advanced-only maximum idle delay between full plan calculations; default 10, range 10 to 1800. Native signals can evaluate earlier. Schema 6 moves the inherited former default of 300 seconds to 10; other customized values, previous seconds, and legacy minutes values migrate automatically.
