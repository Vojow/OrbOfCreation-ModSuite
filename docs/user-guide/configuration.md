...
- Auto Concept considers only discovered (unlocked) concepts that pass its allow/block filters. While a settled training period is still running, its tooltip and Runtime status say that it is waiting. If training has finished but no other unlocked, allowed concept can be assigned, they show that progression-locked reason instead of treating a locked concept as a candidate. The decision journal and trace dashboard record the same idle reason.
...
- `AutoConcept.FallbackEvaluationIntervalSeconds`: Advanced-only maximum idle delay between full plan calculations; default 10, range 10 to 1800. Native signals can evaluate earlier. Schema 6 rewrites every serialized 300-second value — the former default — to 10, including a value deliberately saved by a player; all other customized values, previous seconds, and legacy minutes values migrate automatically.
