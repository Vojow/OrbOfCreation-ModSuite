...
| Crafting live state: visible, starting quantity, `CanBuyAt`, output-capacity verdict, necessary drain ratio/block, time, ordered manual/automation queue contents and amounts, joined resource visibility/quantity/capacity/bandwidth/usage/headroom | 250-ms dynamic | `WorldCraftingRecipeReader` invokes compiled live evaluators; the lifecycle-bound crafting decision reader traverses each pinned page queue once; worker joins current resource rows. Authored lists are not traversed again. |
