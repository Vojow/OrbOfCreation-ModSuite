...
| --- | --- | --- |
| Entity catalog: `uuid`, `name`, optional differing `internalName`, `nativeType`, and addressable `category` | Build-time | Audited `data/entity-mappings.tsv` plus `data/entity-display-names.tsv`, embedded in perf-debug assembly. Decoded once at MCP startup; searches only filter the retained authored rows. Save independence is the catalog contract and is not repeated on every row. |
