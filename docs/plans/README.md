...
| [Auto Buy rejection-aware scheduler](autobuy-rejection-index.md) | Grouped continuation and definite-rejection backoff implemented / runtime gate pending | Structure waits use exact quantity crossings; purchase grouping is independent from repeating ranked passes; NF-03 is enforced; conservative Upgrade handling and Steam Deck profiling remain. |
| [Auto Buy bounded purchase bursts](autobuy-purchase-bursts.md) | Implemented / runtime validation pending | One mutation-owning coordinator lease may submit up to 16 independently revalidated Auto Buy levels within the existing 1 ms slice; native timing, UI pressure, and desktop/Steam Deck gates remain. |
...
| [Versioned configuration schemas](configuration-schema.md) | Next beta / runtime validation pending | Common performs pre-bind, backup-first, rollback-capable schema transactions; Automata owns version 2, Mentor and Mod Config own version 1, and Mod Config reports exact-plugin outcomes separately from runtime health. |
