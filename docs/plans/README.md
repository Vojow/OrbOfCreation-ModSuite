...
| [Versioned configuration schemas](configuration-schema.md) | Next beta / runtime validation pending | Common performs pre-bind, backup-first, rollback-capable schema transactions; supported plugins own version 1 and Mod Config reports exact-plugin outcomes separately from runtime health. |
