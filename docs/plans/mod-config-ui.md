...
Add a **Mods** entry to the game's main navigation immediately after **Time**. The entry opens a mod-owned Unity UI panel rather than trying to express configuration through the game's existing content panels.
...
The first release does not provide a general-purpose game UI framework, edit arbitrary non-BepInEx data, or infer safe live-reload behavior when a mod has not declared it.
...
Selecting Mods hides the current game content panel and shows the mod-owned panel. Leaving Mods restores normal game navigation without changing the selected native view. Unsaved edits prompt for Apply, Discard, or Cancel.

## Architecture

The feature ships as a separate BepInEx plugin, tentatively `OrbModConfig`, with four boundaries:

1. **Navigation host** discovers the main-tab container after the `Main` scene is ready, clones only the minimum button styling needed, inserts Mods after Time, and removes its objects on scene exit or plugin unload.
...
The initial catalog requires no changes or binary dependency in participating mods. Optional presentation metadata can be introduced later through a small, versioned contract after the automatic path is proven. Missing metadata must always fall back to plugin name, config section/key, `ConfigDescription`, acceptable values, default value, and the entry's declared type.
...
### Current implementation status (0.4.0 publication MVP)
...
Version `0.1.0` logged the catalog and UI hierarchy without modifying the interface. Its runtime evidence was used to proceed to the reversible M0 shell; configuration values still remain read-only.
