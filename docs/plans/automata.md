...
Structure and Upgrade completion hooks now retain the completed native family. Structure completion schedules a bounded refresh of the Upgrade registry so newly activated Upgrades can enter ranking promptly; Upgrade completion schedules the corresponding Structure refresh. Multiple events coalesce, while broad cost and availability settlement remains conservative because native completion effects can affect candidates outside the directly refreshed family.
