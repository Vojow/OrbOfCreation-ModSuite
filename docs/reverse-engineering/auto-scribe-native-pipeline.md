...
The checked-in entity catalog contains the closed Auto Scribe identity graph:

| Role | `CraftingRecipeSO` | `ConsumableSO` | `EnchantmentSO` |
|---|---|---|---|
...
Identity mappings do not prove serialized relationships. A game-data extraction or read-only live
probe still has to prove that each recipe emits the listed Scroll, and that each Scroll requests a
structure target and applies the listed enchantment.
...
largest level separation. That conclusion is conditional on the three Scroll assets actually using
this target-selection shape and the expected one-enchantment matcher. Serialized asset evidence is
still required.
...
2. Extract the three recipe assets and prove recipe type, `useQuantityAsLevel`, output Scroll, cost,
   duration, and completion-effect shape.
3. Extract the three Scroll assets and prove family, target reference type, `TargetStructure`
   condition, exact one-role enchantment matcher, and applied enchantment.
