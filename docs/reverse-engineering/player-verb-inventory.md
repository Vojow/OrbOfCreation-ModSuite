...
| `V-DISC-01` discover a generic discoverable | `UIDiscoverablePage.HandleClick` `0x0600231C` -> selected `IDiscoverable.Discover`; concrete members include `TimeRuneSO.Discover` `0x06001858` and `EquipmentSO.Discover` `0x06000B10`; `UICostButton.OnClick` pays before callback | selected concrete UUID/type; visible, not discovered, `CanDiscover`; exact discovery cost | cost paid, discovered flag/effects/dependents change; receipt: type, before/after discovered/created, exact cost, unlocked UUIDs/effects; `EG-05` payment-before-callback partial evidence | **none**; `game_purchase` does not cover `IDiscoverable` | `R4`; `C0+C-DISC+C-PAY`, one test per concrete type | `V-UI-01`; P1 unlock path |
...
| `V-ART-01` create/discover an artifact | artifact-create view uses generic discoverable selection; terminal `EquipmentSO.Discover` `0x06000B10` -> `Create` `0x06000B11`; serialized route is `EG-01` | artifact visible, not created, `CanDiscover`/`CanCreate`, exact discovery/create cost and requirements | created/discovered state, cost, initial equipment instance/effects/attunement eligibility change; receipt: equipment UUID/type, before/after created/discovered/level, exact cost and registered/equipped state | **none**; generic `V-DISC-01` infrastructure may share bindings but artifact receipt is distinct | `R4`; `C0+C-DISC+C-LIST`, no auto-equip unless observed | `V-DISC-01`; P1 artifact progression |

### Spells and targeting

| Verb | UI route and native pipeline | Preconditions/gating | Ordered effects and exact receipts | Existing coverage and named gap | Risk and disposable-save check | Dependencies; playthrough priority |
|---|---|---|---|---|---|---|
| `V-SPELL-01` select glyphs for spell discovery | `UIGlyphList.SelectGlyph` `0x060023F5` -> discovery selection list/event; `SpellManager` holds selected glyph recipe | glyph available; stack/max-usage rules; selection count forms a valid/unknown recipe | ordered/stacked glyph selection changes; receipt: before/after glyph UUID+count list and resolved candidate recipe UUID if any; zero resource/discovery delta | **none** | `R1`; `C0+C-UI+C-LIST`, duplicate/max-use/unknown combo | `V-LVL-01`; P0 prerequisite for buying spells |
| `V-SPELL-02` **buy/discover a selected spell recipe** | `UIDiscoverSpellButton.HandleClick` `0x06002300` event -> `SpellManager.DiscoverSpell` `0x06000741`: find/create empty recipe, `SpellRecipeSO.Discover` `0x06001432`, then `ResourceCostList.PerformCost`; direct recipe button callback -> `SpellManager.DiscoverRecipe` | selected glyph combination resolves; recipe not discovered; `CanDiscover`; exact discovery cost; event wiring partly `EG-01` | native combo path may set discovered **before** payment (`EG-05`), then cost/unlocks; receipt must include selected glyphs, recipe UUID, before/after discovered, exact cost, empty-spell/list effect and discovered-without-payment partial evidence | **none**; known high-priority buy-new-spell gap | `R4`; `C0+C-DISC+C-PAY`, deliberately force post-discover cost divergence in stubs only | `V-SPELL-01`; **P0 HIGH: buy new spells** |
| `V-SPELL-03` create/equip a spell instance | `UICreateSpellButton.HandleClick` `0x060022F9` event -> `SpellManager.CreateSpell` `0x0600073F` -> `CreateRecipe` `0x06000740`; undiscovered path pays/discovers, then creates/adds if slot/usage room via `AddSpell` `0x0600074B` | glyph recipe valid; discovery/payment if needed; spell slot, glyph usage, bandwidth/drain and other requirements | optional discovery cost, new stable spell identity with recipe/glyphs/level, list add, weight/usage/drain and attunement change; receipt: all identities, exact cost, slot/list before/after, usage/drain/attunement and partial step | **none** | `R4`; `C0+C-DISC+C-LIST+C-PAY`, full slots and undiscovered recipe | `V-SPELL-01`,`V-SPELL-02`; P0 equip for casting |
| `V-SPELL-04` remove an equipped spell | spellbook/loadout context -> `SpellManager.RemoveSpell` `0x0600074C` | spell exists in current list; not active/casting/otherwise protected; identity current | removes exact spell, destroys runtime instance, releases glyph/usage/drain, recomputes list; receipt: spell/recipe UUIDs, before/after ordered list and usage/drain, released glyph counts, native refusal | **none** | `R4`; `C0+C-LIST+C-COMBAT`, casting/queued/last-slot cases | `V-SPELL-03`; P1 loadout repair |
| `V-SPELL-05` reorder equipped spell slots | `UISpellList.OnDrop` `0x06002701` -> `SpellListVariable.SwapPositions` + `UpdateObservable` | same list; valid distinct indices | ordered spell identities change only; receipt: before/after ordered stable spell identities and indices; no spell/resource/effect change | **none** | `R1`; `C0+C-LIST`, invalid/cross-list drag and hotkey mapping | `V-SPELL-03`; P1 hotbar control |
| `V-SPELL-06` edit spell level/augment glyph composition | `UISpellRecipeButton.AttachSpell` `0x0600270D`; `UISpellInformation.SetSpellLevel` `0x06002535`; `UIGlyphList.SelectGlyph/OnDrop` adjusts selected augment stack | spell/recipe selected; level <= purchased max; glyph available/max-use; slot/usage/bandwidth/drain constraints | selected level and ordered/stacked augments change, usage/cost/requirements recompute; receipt: spell identity, before/after level+glyph counts, usage/drain/availability; partial captures composition applied but invalid derived state | **none** | `R4`; `C0+C-LIST`, cap/duplicate/unavailable glyph and derived-state reconciliation | `V-SPELL-03`,`V-LVL-01`; P1 spell customization |
...
| `V-TGT-01` select and submit a specific target | `TargetingManager.RequestTarget` `0x0600076E`; `UICharacterList.ClickCharacter` `0x06002269` event -> `TargetingManager.SubmitTarget` `0x06000775`; listener is `EG-01` | active target request; candidate stable identity is alive, eligible, and type-compatible | target link/candidate changes and submit may release the requesting action; receipt: request/action ID, target stable identity/type, before/after target state and downstream action receipt | **partial**: `game_cast` accepts an audited target, but no generic target-request capability serves rituals/other verbs | `R3`; `C0+C-COMBAT`, invalid/dead/stale candidate | requesting verb, chiefly `V-SPELL-07`; P1 |
| `V-TGT-02` randomize a pending target | `UITargetingInterface.Randomize` `0x0600276E` -> current targeting link randomization | active target request; eligible random pool nonempty | selected candidate changes without submitting; receipt: request ID, eligible pool identities, before/after candidate, RNG/result evidence and zero downstream action delta | **none** | `R2`; `C0+C-COMBAT`, empty/single/multiple pools | `V-TGT-01`; P2 |
| `V-TGT-03` cancel a pending target request | targeting close/cancel control -> `UITargetingInterface.Close` `0x0600276C`; terminal request-cancel listener is `EG-01` | active pending target request; requesting action has not committed | targeting UI/request closes and requesting action remains uncommitted; receipt: request/action ID, before/after pending state, zero cost/queue/effect delta | **none** | `R2`; `C0+C-COMBAT`, no-request/idempotent and queued-action cases | `V-TGT-01`; P1 |
...
| Verb | UI route and native pipeline | Preconditions/gating | Ordered effects and exact receipts | Existing coverage and named gap | Risk and disposable-save check | Dependencies; playthrough priority |
|---|---|---|---|---|---|---|
| `V-CRAFT-01` execute a one-shot manual recipe | `UICraftingRecipeList.ClickCraft` `0x060022E1` -> optional page callback -> `CraftingRecipeSO.Execute`; manual `UICraftingPage.QueueCraft` `0x060022F0` may create/init/queue an instance | exact concrete recipe identity/type; visible/available/can craft; exact inputs; quantity/level; queue/instant rules | exact input cost, output quantity, recipe progress/mastery and optional instance/queue change; receipt: recipe, requested/accepted level+quantity, exact input/output deltas, before/after instance/queue/progression and partial step | **partial**: `AutoScribeOneShotCraftGameAction` fully handles six Scribe recipes feature-side but has no MCP; all other crafting recipes have no action | `R3/R4`; `C0+C-PAY+C-QUEUE`, instant/queued and each Scribe recipe | `V-LVL-01`,`V-UI-06`; P1, especially Craft Sigils/Scribe |
...
| `V-CONS-01` use a consumable | `UIConsumableRefList.ClickConsumable` `0x0600229B` event -> `ConsumableSO.SelectAndFire` -> quantity/multi-buy queue; `Fire` starts usage and cost | inventory quantity; visible/available; queue/capacity/target/cooldown gates | inventory/cost decreases, usage queue/effects/target change; receipt: consumable+target identities, requested/queued/used quantity, exact stock/resource/queue/effect before/after | **partial**: `AutoItemsConsumableUseGameAction` fully handles the audited automation family but has no MCP/general invocation | `R3`; `C0+C-LIST+C-QUEUE+C-COMBAT` | `V-TGT-01`,`V-UI-06`; P1 |
| `V-CONS-02` cancel queued consumable use | `UIConsumableRefList.CancelConsumable` `0x0600229D` -> `ConsumableSO.CancelUsage` | queued/active usage; cancellable | queue/effect removed and inventory/cost may refund (`EG-02`); receipt: before/after stock/queue/effects and exact refund delta | **none** | `R3`; `C0+C-QUEUE+C-PAY` | `V-CONS-01`; P2 |
| `V-CONS-03` discard consumables | `UIConsumableRefList.DiscardConsumable` `0x0600229E` -> multi-buy `ConsumableSO.Discard` | inventory quantity; discard mode/confirmation where authored; amount clamped | stock permanently decreases; receipt: UUID/type, requested/discarded and exact before/after quantity; no use effect/resource refund unless native proves one | **none** | `R4`; `C0+C-LIST`, one/all/zero and confirmation | `V-UI-06`; P4 |
| `V-CONS-04` toggle consumable randomization | `UIConsumableRefItem.TurnRandomizationOn/Off` `0x06002296/97` | consumable supports randomization; row available | per-consumable randomization flag changes; receipt: UUID/type and before/after flag; no immediate stock/effect delta | **none** | `R2`; `C0+C-UI`, unsupported and reload (`EG-03`) | `V-CONS-01`; P3 |
| `V-CONS-05` reorder consumables | `UIConsumableRefList.OnDrop` `0x0600229A` -> list reorder | valid same-list distinct indices | ordered consumable reference UUIDs change; receipt: before/after order and indices, unchanged quantities | **none** | `R1`; `C0+C-LIST` | none; P3 |
| `V-EQ-01` equip/increase an artifact stack | equipment list click event (`UIEquipmentList.SelectEquipment` `0x060023CE`, `EG-01`) -> `EquipmentManager.ToggleItem/EquipItem` `0x06000514-17`: cost/slot/type/max/multi-buy clamp, list stack, `EquipmentSO.Equip` | artifact created; usage cost; global/type slot room or already stacked; below max; current identity | equipped stack and ordered list, usage/effects and attunement change; receipt: equipment UUID/type, requested/equipped stacks, before/after list+usage+effects+attunement; partial stack/effect disagreement | **none** | `R3/R4`; `C0+C-LIST+C-PAY`, new stack/increase/full slots/attuning | `V-ART-01`,`V-LVL-01`; P1 |
| `V-EQ-02` unequip/decrease an artifact stack | equipment click -> `EquipmentManager.UnEquipItem` `0x06000518/19`: clamp multi-buy, unstack, refresh, call `EquipmentSO.Equip(remaining)` | equipment currently equipped; positive stack | stack/list decreases, usage/effects released/recomputed; receipt: requested/removed/remaining, before/after list+usage+effects+attunement | **none** | `R3`; `C0+C-LIST`, some/all and last/type-slot effects | `V-EQ-01`; P1 |
...
| `B-002` | `V-SPELL-01`, `V-SPELL-02`, `V-SPELL-03` | Spell-manager discovery/create family: select glyph recipe, buy/discover it, create/equip a stable spell instance. This is the shortest route from an offer/unlock to an MCP-castable spell. |
| `B-003` | `V-SPELL-06` | Spell composition action for output level and augment stacks; depends on the created stable spell identity from `B-002`. |
| `B-004` | `V-SPELL-04`, `V-SPELL-05` | Spell-list lifecycle: remove and reorder; needed to repair full loadouts and control hotkey slot identity. |
...
| `B-007` | `V-CRAFT-01` | One-shot crafting family. Expose the existing Auto Scribe action through MCP first, then widen by complete concrete recipe bindings; this unlocks Craft Sigils without duplicating Scribe semantics. |
| `B-008` | `V-DISC-01` | Generic discoverable family, after the higher-value offer and spell pipelines establish receipt conventions. |
| `B-009` | `V-ART-01`, `V-EQ-01`, `V-EQ-02` | `EquipmentSO` creation/equip lifecycle; artifact creation and stack application share stable equipment identity/effects and enable artifact loadouts. |
