...
| `V-RES-01` start/queue research development | `UIResearchItem.DevelopResearch` `0x060025F0` -> `ResearchSO.PurchaseLevel` `0x060011B4` -> setting-dependent `Develop` `0x060011B5` or `QueueDevelopment` `0x060011B6` | exact published `ResearchSO`; immediate route requires native `CanDevelop` and exact `HasEnough`; queue route replays native multi-buy/max clamp and cumulative per-level cost/range loop | exact target enters active development or its total queued levels increase; success returns the complete newer named research row with route, accepted levels, exact named costs/current holdings, investment/progress, bonus capacity, and every next verb; payment/investment deltas are observations | **full**: `game_research(mode=develop)` / `ResearchGameAction`, with the [audited dossier](research-development-native-pipeline.md) | `R3`; `C0+C-PAY+C-QUEUE`, both immediate/queued modes and partial affordable prefix | `V-UI-08`; P1 progression |
| `V-RES-02` pause or resume research | `UIResearchItem.PauseResearch/ResumeResearch` `0x060025F1/F2` -> `ResearchSO.PauseResearch/ResumeResearch` | exact target developing in the matching active/paused state; the UI exposes these only with Research Queue Mode disabled | exact target remains developing and becomes inactive/active; success returns progress, investment, state, and all next decisions inline; resource and progress-clock changes are observations | **full**: `game_research(mode=pause|resume)` / the same `ResearchGameAction` and dossier | `R2`; `C0+C-QUEUE`, both states and lifecycle invalidation | `V-RES-01`; P2 |
| `V-RES-03` cancel research development | `UIResearchItem.CancelDevelopment` `0x060025F7` -> `ResearchSO.CancelDevelopment` `0x060011B7`; clears development investment | exact target is developing; lifecycle and ownership permit remain current | exact target is no longer developing and has zero total queued levels; success returns the idle row and next develop/bonus decisions; cleared investment and any resource disposition are evidence, never outcome gates | **full**: `game_research(mode=cancel)` / the same `ResearchGameAction` and dossier | `R3`; `C0+C-QUEUE+C-PAY` at zero/partial/near-complete progress | `V-RES-01`; P2 |
| `V-RES-04` apply a research bonus level | `UIResearchItem.AddBonusLevel` `0x060025F8` -> `ResearchSO.CanApplyBonusLevels` -> `SubmitBonusLevel` `0x060011BA` | target is not developing; the native predicate sees at least one associated `ResearchTypeSO.HasFreeBonusLevelsLeft`; free capacity is positive | exact target's self-bonus level increases by one; success returns purchased/base/bonus/total levels plus remaining named type capacity and next decisions; type counters/effects are evidence | **full**: `game_research(mode=bonus)` / the same `ResearchGameAction` and dossier | `R4`; `C0+C-PAY`, explicitly distinguish purchased from bonus | `V-RES-01`; P2 |
...
| Verb | UI route and native pipeline | Preconditions/gating | Ordered effects and exact receipts | Existing coverage and named gap | Risk and disposable-save check | Dependencies; playthrough priority |
|---|---|---|---|---|---|---|
| `V-CHAL-01` select/unselect an offered challenge | `UIChallengeItem.ToggleSelection` `0x0600224E` -> selected challenge list `Toggle` | exact challenge is in a current offer or already selected; native selection room and compatibility are rechecked | exact target membership in the ordered selection inverts; success returns the complete newer named challenge row and shared challenge decision state | **full**: `game_challenge(mode=select)` / `ChallengeGameAction`, with the [audited dossier](challenge-native-pipeline.md) | `R2`; `C0+C-RESET`, incompatible/cap/stale offer | `V-CHAL-04`; P1 before prestige |
| `V-CHAL-02` queue/unqueue challenge activation | `UIChallengeItem.FireActionButton` `0x0600224B` inactive branch -> `ToggleActivate` `0x0600224C` -> `ChallengeSO.ToggleQueueActivation` | exact challenge is currently offered and state is idle or queued | exact target state toggles idle/queued; success returns the target's state, next difficulty/reward, selection and all next decisions; projected effects remain observations | **full**: `game_challenge(mode=queue)` / the same `ChallengeGameAction` and dossier | `R3`; `C0+C-QUEUE+C-RESET`, queue/unqueue and reload | `V-CHAL-01`; P1 |
| `V-CHAL-03` abandon an active challenge | `UIChallengeItem.FireActionButton` active branch -> `AbandonActivation` `0x0600224D` -> `ChallengeSO.AbandonChallenge` | exact challenge is currently active | exact target enters native Failed state; success returns the complete newer row and challenge state; reward/effect/progress disposition is observation and never replaces the target outcome gate | **full**: `game_challenge(mode=abandon)` / the same `ChallengeGameAction` and dossier | `R4`; `C0+C-RESET`, no-progress/partial/complete cases | `V-CHAL-02`; P2 |
| `V-CHAL-04` reroll/fetch challenge offers | `UITimeScreenManager.FetchNewChallenges` `0x06002444` or `UIPersistentResetModal.FetchNewChallenges` `0x060024EE` commits first-fetch/reroll state then calls `ChallengeManager.LoadNewActiveChallenges` or `PersistentResetManager.FetchNewChallenges`; each callback cycles out then instantiates/queues offers | world cycle complete; first fetch or rerolls positive; native managers and lists available; no screen-render dependency | requested Time/prestige list becomes non-empty with every offer queued; success returns both ordered fully named offer lists, selections, rerolls, and both next fetch decisions | **full**: `game_challenge(mode=fetch_time|fetch_prestige)` / the same `ChallengeGameAction` and dossier | `R4`; `C0+C-RESET`, first/subsequent/zero/exhausted/selected offer | `V-UI-01`; P1 |
| `V-PREST-01` perform a persistent reset/prestige | reset-confirm UI -> `PersistentResetManager.PersistentReset` `0x06000651`, whose fade callback invokes `PersistentResetLogic` `0x06000652`: setup, `GameManager.PersistentResetGameState`, challenge reward/activation, persistent-resource set, `CleanGame` scene reload | published world-cycle completion and fetched challenge choices; required `confirm:true`; lifecycle current; complete native bindings; exclusive permit captured last | exact lifecycle replacement is the outcome gate; success waits for a newer post-reset world and returns its scene, named prestige holding/projection, reset availability, challenge selections/offers, and next decisions; counters/resources are not ledger gates; throw or missing lifecycle transition quarantines | **full**: `game_prestige(confirm=true)` / `PrestigeGameAction`, with the [audited dossier](persistent-reset-native-pipeline.md) | `R5`; `C0+C-RESET+C-SAVE`; disposable-save promotion remains mandatory | `V-CHAL-01`,`V-CHAL-02`; P1 required for full loop |
...
| `B-010` | `V-CHAL-01`, `V-CHAL-02`, `V-CHAL-03`, `V-CHAL-04` | **Complete:** `game_challenge` and `ChallengeGameAction` select, queue, abandon, and fetch either challenge-offer surface through the audited manager/list pipelines; challenge rows plus shared challenge state expose ordered named offers/selections, rerolls, caps, exact next difficulty/reward, and every next decision. |
| `B-011` | `V-PREST-01` | **Complete:** `game_prestige(confirm=true)` and `PrestigeGameAction` revalidate the UI admission facts, invoke the audited synchronous reset transaction without a rendered-fade dependency, verify lifecycle replacement, and return a fresh named post-reset prestige/challenge decision surface. |
| `B-012` | `V-RES-01`, `V-RES-02`, `V-RES-03`, `V-RES-04` | **Complete:** `game_research` and `ResearchGameAction` develop/queue, pause, resume, cancel, and apply bonus levels through the audited native lifecycle. Research rows publish the exact route, accepted cumulative cost prefix with named holdings, progress/investment, free bonus capacity, and all next decisions; committed calls return that complete newer row inline. |
