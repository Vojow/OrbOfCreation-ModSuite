...
internal sealed class DiagnosticsBundleControlView : IDisposable
{
    private const float RowGap = 8f;
    private const float RowInset = 12f;
    private const float MinimumRowHeight = 112f;
    private readonly IDiagnosticsBundleControl _control;
    ...
    internal DiagnosticsBundleControlView(
        RectTransform parent,
        TextMeshProUGUI template,
        IDiagnosticsBundleControl control)
    {
        ...
        _buttonLabel = _button.GetComponentInChildren<TextMeshProUGUI>() ??
            throw new InvalidOperationException("The bug-report button has no label.");
    ...
    {
        var presentation = DiagnosticsBundlePresenter.Build(_control.Status, _control.BundleRequested);
    ...
    {
        if (_control.RequestBundle() != DiagnosticsBundleRequestResult.Accepted) return;
        var presentation = DiagnosticsBundlePresenter.Build(_control.Status, _control.BundleRequested);
        _buttonLabel.text = presentation.ButtonLabel;
        _button.interactable = presentation.ButtonEnabled;
    }
