...
internal sealed class PumpTimingGraphView : IDisposable
{
    ...
    public void Refresh()
    {
        ...
        var scaleTicks = PumpTimingGraphProjection.ScaleTicks(_sortedTicks.AsSpan(0, count));
        var overScale = 0;
        for (var index = 0; index < count; index++)
            if (_sortedTicks[index] > scaleTicks) overScale++;
        var columnCount = PumpTimingGraphProjection.Build(
            _samples.AsSpan(0, count),
            _columns);
        _graphic.SetColumns(_columns, columnCount, scaleTicks);
        _summary.text = string.Format(
            CultureInfo.InvariantCulture,
            "{0} exact frames (~{1:F1}s @ 60 FPS)  |  avg {2:F3} ms  |  p95 {3:F3} ms  |  max {4:F3} ms\nScale {5:F3} ms (p99); {6} frame(s) above it drawn clipped in red  ·  Gray idle  ·  Blue capture attempt  ·  Green response  ·  Orange action",
            count,
            count / FramesPerSecondReference,
            Milliseconds(totalTicks / (double)count),
            Milliseconds(p95Ticks),
            Milliseconds(maximumTicks),
            Milliseconds(scaleTicks),
            overScale);
    }
