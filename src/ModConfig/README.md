...
Unsupported custom setting types remain read-only. Closing the panel preserves staged values for the current scene; Revert explicitly discards only the selected mod's staged values. The Mods shell and Safety settings remain available when `General.Enabled` is false.
