...
public readonly struct DecisionJournalStatus : IEquatable<DecisionJournalStatus>
{
        ...
        string faultMessage = "")
    {
        ...
        if (bytesWritten < 0 || writtenSegments < 0 || writtenSegments > writtenRecords ||
            retainedSegments < 0 || evictedSegments < 0 || startupPrunedSegments < 0 ||
            incompatibleSegmentsPruned < 0 || staleTemporaryFilesRemoved < 0 || pendingBlocks < 0 ||
        ...
        if (unavailable != (artifactName.Length == 0) ||
            ...
            unavailable && !HasZeroMetrics(
                ...
                incompatibleSegmentsPruned,
                staleTemporaryFilesRemoved,
                pendingBlocks,
                peakPendingBlocks,
                firstIncompleteSequence) ||
            unavailable && (faultSite.Length != 0 || faultMessage.Length != 0) ||
            faultMessage.Length != 0 && faultSite.Length == 0 ||
    ...
    /// <summary>Segments abandoned at startup because this build could not continue the store.</summary>
    ...
    /// <summary>The observation the journal died in, empty when nothing named one.</summary>
    public string FaultSite { get; }

    /// <summary>The base message of the exception that stopped the journal, empty when none.</summary>
    ...
    public bool Equals(DecisionJournalStatus other) =>
        ...
        IncompatibleSegmentsPruned == other.IncompatibleSegmentsPruned &&
        ...
        string.Equals(FaultSite, other.FaultSite, StringComparison.Ordinal) &&
        string.Equals(FaultMessage, other.FaultMessage, StringComparison.Ordinal);
    ...
    private static bool HasZeroMetrics(
        ...
        startupPrunedSegments == 0 && incompatibleSegmentsPruned == 0 &&
        staleTemporaryFilesRemoved == 0 && pendingBlocks == 0 &&
