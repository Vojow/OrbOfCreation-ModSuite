...
internal sealed partial class ServiceCycleSemanticExecutionEvents
{
    internal void StartAttemptObserved(int ordinal, in ServiceCycleStartAttempt attempt)
    {
        ...
        if (attempt.CaptureFact.IsPresent)
        {
            ...
            else if (capture.Result.IsCaptured)
            {
                ...
                _publications.EnsureStrategy(context.Strategy, capture.CompletedAt);
        ...
        _publications.EnsureConfiguration(cycle.Config, attempt.QueuedAt);
        _publications.EnsureStrategy(cycle.Strategy, attempt.QueuedAt);
    ...
    {
        _publications.EnsureConfiguration(context.Config, context.CapturedAt);
