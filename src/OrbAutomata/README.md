...
Orb Automata is a BepInEx 5 automation suite for Orb of Creation. Version `0.8.8` adds a fail-closed versioned configuration transaction before normal typed binding. It routes native automation through fail-closed family adapters and normalized admission facts while keeping queue-filling Auto Buy responsive during rapid native completions. Structure and Upgrade purchases use separate exact-signature adapters; Auto Cast rejects an entire cost vector when any native entry is undecodable; and an unaudited game-assembly hash disables native mutations instead of only warning. Bounded candidate/resource circuits keep transient native failures from consuming every pass, independent action-family ownership isolates known overlap, and capture-execute-capture postconditions still verify every mutation. It also provides Auto Cast, opt-in Auto Concept rotation, progression-aware spell leveling, and shared lifecycle generation isolation.

Automata claims Structure, Upgrade plus native multi-buy override, Spell Cast, Concept Assignment, and Spell Level Purchase independently. The exact AutobuyOrb GUID blocks only Structure and Upgrade automation. Claims are released on configuration/lifecycle teardown, prepared work is cancelled on loss, and ownership is rechecked after live native validation immediately before mutation. Unknown unregistered automation is not disabled and cannot be proven absent.
...
The former runtime-probe, per-session purchase-limit, DryRun, expert-override, and Auto Research settings are not part of the release configuration or Mod Config UI. The schema-zero migration explicitly discards the reviewed obsolete keys, maps `AutoConcept.Mode=BalanceMastery` to `Active`, and moves the reviewed seconds/minutes interval keys to `FallbackEvaluationIntervalSeconds` with destination-then-seconds-then-minutes precedence. Minutes use invariant parsing, round away from zero, and the result is clamped to 10-1800 seconds. A migration creates the first free sibling `.bak` backup before mutation; malformed, negative, or future schema data fails closed without starting Automata.
