...
internal sealed class AutoBuyToggleButton : IDisposable
{
    ...
    public void Render()
    {
        ...
        var label = state == AutoCastToggleVisualState.On ? "ON" : state == AutoCastToggleVisualState.Blocked ? "!" : state == AutoCastToggleVisualState.Waiting ? "~" : "OFF";
        var color = state == AutoCastToggleVisualState.On ? new Color(.4f, 1, .55f) : state == AutoCastToggleVisualState.Blocked ? new Color(1, .35f, .3f) : state == AutoCastToggleVisualState.Waiting ? new Color(1, .75f, .35f) : new Color(.7f, .7f, .7f);
