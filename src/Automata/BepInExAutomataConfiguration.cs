...
internal sealed class BepInExAutomataConfiguration
{
    ...
    private BepInExAutomataConfiguration(
        ...
        ConfigEntry<bool> autoItemsScrolls,
        ...
        ConfigEntry<float> relativeReserveMultiplier)
    {
        ...
        AutoItemsScrolls = autoItemsScrolls;
    ...
    public ConfigEntry<bool> AutoItemsScrolls { get; }
    ...
    internal static BepInExAutomataConfiguration BindCurrent(ConfigFile config)
    {
            ...
            var result = new BepInExAutomataConfiguration(
                ...
                Bind(config, "AutoItems", "Mode", AutoItemsOperationMode.Disabled, "Disabled performs no item work. Active considers enabled exact native item families under the toxicity policy.", 19, 0),
                Bind(config, "AutoItems", "UseScrolls", true, "Allow exact Scroll-family items through the game's native random-target path.", 19, 10, dependencies: AutoItemsActiveDependencies),
                ...
                Bind(config, "AutoItems", "EvaluationIntervalSeconds", 1.0f, "Idle and rejected-action evaluation interval. Verified item chains resume immediately after fresh world publication, while a busy native consumable queue is checked at least every 0.25 seconds.", 19, 60, new AcceptableValueRange<float>(0.25f, 10.0f), AutoItemsActiveDependencies),
                Bind(config, "AutoScribe", "Mode", AutoScribeOperationMode.Disabled, "Disabled prepares no Scrolls. Active rotates through every enabled audited Scribe role and supplies each at that recipe's highest currently affordable level.", 20, 0),
