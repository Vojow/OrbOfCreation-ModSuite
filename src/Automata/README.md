...
`FallbackEvaluationIntervalSeconds` is an Advanced setting, not a rotation delay. It defaults to 10 seconds and accepts 10 through 1800 as the maximum idle delay between full plan calculations; native changes can request earlier passes. Schema 6 rewrites every serialized 300-second value — the former default — to 10, including a value deliberately saved by a player; all other customized values are preserved. Existing `RebalanceIntervalSeconds` and `RebalanceIntervalMinutes` values migrate automatically.
