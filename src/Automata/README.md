...
The former runtime-probe, per-session purchase-limit, DryRun, expert-override, Auto Research, and Auto Harvest runtime-selector settings are not part of the release configuration or the configuration UI. Automation has no configuration schema of its own: it binds into the suite's single configuration file, which the shared pre-bind transaction marks at `ConfigurationSchemaVersion` 7. The 2-to-3 step moves only Auto Cast's inherited `Left Alt + X` default to `F8`; the 3-to-4 step discards Mentor's retired frame-operation and CPU-budget keys; the 4-to-5 step discards the retired scan, logging, event-probe, and verifier controls; the 5-to-6 step shortens Auto Concept's inherited idle fallback to 10 seconds; the 6-to-7 step shortens its inherited training period to 30 seconds. Player-customized active shortcuts and intervals are preserved. Malformed, negative, or future schema data fails closed without starting the suite.
...
`FallbackEvaluationIntervalSeconds` is an Advanced setting, not a rotation delay. It defaults to 10 seconds and accepts 10 through 1800 as the maximum idle delay between full plan calculations; native changes can request earlier passes. Schema 6 moves the inherited former default of 300 seconds to 10 while preserving other customized values. Existing `RebalanceIntervalSeconds` and `RebalanceIntervalMinutes` values migrate automatically.
...
Successful Auto Concept initialization records separate scoped-recipe, active-loadout, and eligible-candidate counts. While an active assignment is settling or training, the feature tooltip and Mods Runtime report that wait. After training, a timed cycle with no other unlocked, allowed assignment reports a progression-locked status; locked concepts remain excluded rather than delaying or satisfying the rotation. Runtime trace, the decision journal, and the trace dashboard carry the same idle reason and distinguish assignment reservation, settled training start, catch-up or timeout completion, rotation, and an idle balancer from a missed evaluation.
