...
- Each ranked Structure receives one live Bulk Development-sized group and each Upgrade receives one level. Every submitted level is independently revalidated.
...
A value that could not be read is not evidence. A candidate whose every cost row prices at zero has not been shown to be affordable, only to be unpriceable, so it is excluded rather than bought — the failure direction that once planned all 180 structures at once after a cold load. One free row on an otherwise priced candidate is different and is simply skipped, because relative to the rows that did price it really is free. A Structure action requests the game's live Bulk Development count; an Upgrade requests one level. The action boundary clamps every request to queue room above the configured reserve, since the game queues one entry per level.
...
Auto Buy always plans the complete ranked candidate list and lets the native boundary continue until only `LeaveQueueSlots` remain. Structures follow the live Bulk Development count and Upgrades always request one level. Exact rising grouped costs are reserved in the batch ledger, and every group is capped to usable queue room.
...
A live slot or prospective-drain refusal defers only that candidate for the world/configuration
publication that produced the rejected plan. Timed ordering immediately considers the next unlocked
candidate against the same pinned facts; if every replacement is deferred, safe depth remains
eligible and the service waits for its existing publication wake. A newer world or configuration
publication makes the candidates eligible again without a retry deadline, fallback poll, or second
wake path. The tooltip and Runtime status identify this native-safety publication wait. Verified
quantity changes and rejected preflights are written to `BepInEx/LogOutput.log` with the affected
recipe UUIDs and native reason.
