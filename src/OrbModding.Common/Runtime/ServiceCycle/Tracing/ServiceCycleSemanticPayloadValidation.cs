...
internal static partial class ServiceCycleSemanticPayloadValidation
{
    internal static void EnsureValid(ServiceCycleSemanticEventKind kind, in ServiceCycleSemanticPayload payload)
    {
        ...
        var exactShape = payload.Fields == expected;
        if (kind == ServiceCycleSemanticEventKind.ActionFaulted)
            exactShape |= payload.Fields == (expected | ServiceCycleSemanticFields.NativeMutationOutcome);
        if (!exactShape)
