    ...
    where TConfig : notnull
{
    ...
    private readonly ServiceResourceClaim? _frameClaim;
    private readonly IServiceCycleWorkerExitObserver? _exitObserver;
    private ServiceCycleWorkerState<TFrame, TConfig, TState, TAction> _workerState;
        ...
        IServiceCycleWorkerExitObserver? exitObserver)
    {
        ...
        _frameClaim = frameClaim;
        _exitObserver = exitObserver;
        if (_frame.Lifecycle != lifecycle || (_handoff.Lifecycle.Value != 0 && _handoff.Lifecycle != lifecycle))
        ...
        _workerState = new ServiceCycleWorkerState<TFrame, TConfig, TState, TAction>(
            _definition,
            _resourceClaims,
            _clock);
