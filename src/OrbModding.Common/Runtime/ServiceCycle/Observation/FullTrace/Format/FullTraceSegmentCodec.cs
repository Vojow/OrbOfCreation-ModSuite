...
internal static class FullTraceSegmentCodec
{
    ...
    internal const int MaximumRecords = 3_854;
    ...
    internal static int GetEncodedLength(int recordCount)
    {
        ...
        return checked(HeaderBytes + checked(recordCount * ServiceCycleSemanticEventV5Codec.RecordBytes) + FooterBytes);
    }
        ...
        Span<byte> destination)
    {
        ...
        WriteU16(output, 10, ServiceCycleSemanticEventV5Codec.RecordBytes);
        ...
        WriteU64(output, 64, checked((ulong)events.Length * ServiceCycleSemanticEventV5Codec.RecordBytes));
        ...
        for (var index = 0; index < events.Length; index++)
        {
            ServiceCycleSemanticEventV5Codec.Write(
                output.Slice(
                    HeaderBytes + index * ServiceCycleSemanticEventV5Codec.RecordBytes,
                    ServiceCycleSemanticEventV5Codec.RecordBytes),
                in events[index]);
        }
    ...
    {
        if (source.Length < HeaderBytes + ServiceCycleSemanticEventV5Codec.RecordBytes + FooterBytes ||
            ...
            ReadU16(source, 10) != ServiceCycleSemanticEventV5Codec.RecordBytes ||
        ...
        if (sessionValue == 0 || semanticSessionValue == 0 || firstTransportSequence == 0 ||
            ...
            payloadBytes != checked(countValue * ServiceCycleSemanticEventV5Codec.RecordBytes) ||
        ...
        {
            events[index] = ServiceCycleSemanticEventV5Codec.Read(source.Slice(
                HeaderBytes + index * ServiceCycleSemanticEventV5Codec.RecordBytes,
                ServiceCycleSemanticEventV5Codec.RecordBytes));
        }
        ValidateEvents(semanticSession, serviceCapacity, events);
