...
public readonly struct ManualFullTraceStatus : IEquatable<ManualFullTraceStatus>
{
        ...
        string artifactName)
    {
        ...
        if (inactive && (duration != TimeSpan.Zero || acceptedRecords != 0 || writtenRecords != 0 ||
                ...
                manifestCommitted || result != ManualFullTraceResult.None || artifactName.Length != 0) ||
    ...
    public override int GetHashCode() => HashCode.Combine(
        ...
        BytesWritten,
        SegmentCount,
        HashCode.Combine(FirstIncompleteSequence, ManifestCommitted, Result, ArtifactName));
    ...
    private static ManualFullTraceStatus CreateInactive(ManualFullTraceState state) =>
        new(state, TimeSpan.Zero, 0, 0, 0, 0, 0, false, ManualFullTraceResult.None, string.Empty);
}
