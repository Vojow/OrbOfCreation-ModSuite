...
internal sealed class SuiteFramePumpEvidenceEmitter
{
        ...
        long frameIdentity)
    {
        ...
        if (trace is not null)
        {
            ...
            var profile = _profiler.Begin(
                ServiceCycleProfileCommonStageCodes.SemanticTerminal,
                ordinal,
        ...
        long frameIdentity)
    {
        ...
        if (trace is not null)
        {
            ...
            var profile = _profiler.Begin(
                ServiceCycleProfileCommonStageCodes.SemanticStart,
                ordinal,
                lifecycle,
                cycle,
                frameIdentity);
...
#if SERVICE_CYCLE_PROFILE
            var profile = _profiler.Begin(
                ServiceCycleProfileCommonStageCodes.SemanticPumpSummary,
                serviceOrdinal: 0,
                currentLifecycle.Value,
                cycle: 0,
