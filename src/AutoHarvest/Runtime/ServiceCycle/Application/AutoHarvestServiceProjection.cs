...
internal static class AutoHarvestServiceProjection
{
        ...
        out AutoHarvestPairHealth health)
    {
        ...
        // The upper bound is the last member of AutoHarvestPairHealthKind; adding one means moving it
        // here too, or the new kind reads back as an unreadable projection.
        if (selected.Kind != ServiceProjectionValueKind.Boolean ||
            ...
            kind.Integer is < (int)AutoHarvestPairHealthKind.NotSelected or
                > (int)AutoHarvestPairHealthKind.PrerequisitesNotConfirmed)
        {
            health = default;
            return false;
        }
