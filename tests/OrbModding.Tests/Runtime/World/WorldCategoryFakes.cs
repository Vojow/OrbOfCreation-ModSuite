...
/// </summary>
internal sealed class FakeEffectProperty
{
    public List<FakeResourceEffect> resourceEffects = new();
    public List<FakeObjectEffect> upgradeableObjectEffects = new();
}

/// <summary>An effect on one of a resource's modifiable properties, named by the game's enum.</summary>
internal struct FakeResourceEffect
{
    public FakeReferencedEntity resource;
    public FakeModifiableProperty upgradeType;
    public FakeValueModifier modifier;

    internal FakeResourceEffect(
        Guid resourceId,
        FakeModifiableProperty property,
        FakeModifierKind kind,
        double amount)
    {
        resource = new FakeReferencedEntity { Identity = resourceId };
        upgradeType = property;
        modifier = new FakeValueModifier(kind, amount, 0);
    }
}

/// <summary>
/// An effect on a named property of any upgradeable object. The name is a string here because it is
/// one in the game: each type authors its own property names rather than sharing an enum.
/// </summary>
internal struct FakeObjectEffect
{
    public FakeReferencedEntity upgradeableObject;
    public string propertyType;
    public FakeValueModifier modifier;
    public bool useTargetRef;

    internal FakeObjectEffect(
        Guid targetId,
        string propertyType,
        FakeModifierKind kind,
        double amount,
        bool useTargetRef = false)
    {
        upgradeableObject = new FakeReferencedEntity { Identity = targetId };
        this.propertyType = propertyType;
        modifier = new FakeValueModifier(kind, amount, 0);
        this.useTargetRef = useTargetRef;
    }
}

/// <summary>
/// A stand-in for the game's <c>ResourceSO.ModifiableType</c>. Member names matter here, unlike every
/// other enum a fake carries: the collector publishes the name rather than the integer, because the
/// other vocabulary reaching that table has no integer to share.
/// </summary>
internal enum FakeModifiableProperty
{
    Rate,
    MaxQuantity,
    Quality,
