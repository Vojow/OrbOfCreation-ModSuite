...
// Vendored third-party source: keep the upstream body byte-faithful rather than annotating it
// for this repo's nullable context.
#nullable disable

// ---------------------------------------------------------------------------
// Vendored from BreakInfinity.cs — https://github.com/Razenpok/BreakInfinity.cs
// MIT License — Copyright (c) 2020 Andrei Andreev. Full text in BreakInfinity.LICENSE.
//
// This is the ACTUAL BreakInfinity source the game ships (Assembly-CSharp-firstpass),
// placed in the GLOBAL namespace exactly as the game does, so this test double's
...
// ---------------------------------------------------------------------------
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        // Game-additive surface (Assembly-CSharp-firstpass exposes these on its BigDouble):
        // Ma/Ex are raw get+set accessors over the mantissa/exponent (no re-normalization on set,
        // matching the game's set_Ma/set_Ex); ToInt/ToLong/ToFloat narrow ToDouble().
        public double Ma { get => mantissa; set => mantissa = value; }
